...
//(GUARDA EN VARIABLES ORDINARIAS, HAY QUE GRABAR EN MEMORIA DEL ARDUINO O RTC)
//----------------------------------------------------
void setear_alarma(){
     h=0;
     m=0;
...
//-----------------------------------------------------
void leer_alarma(){
     h=EepromRTC.readInt(2); //EepromRTC.readInt lle un valor entero
     m=EepromRTC.readInt(4); // de la dirección de memoria pasada como
     s=EepromRTC.readInt(6); // parámetro y se guarda el valor en una variable del mismo tipo
     return;
}
//-----------------------------------------------------
//-----------------------------------------------------
...
//-----------------------------------------------------
void loop() {    
     DateTime now = rtc.now(); // Obtener fecha actual 
